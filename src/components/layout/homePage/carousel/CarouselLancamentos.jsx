...
import { useCarouselLancamentos } from '../../../functions/homePage/carousel/CarouselFunctions';
import {
  ButtonAdicionarCarrinho,
  ContainerButtonsFunctions,
} from '../../../../styles/CategoriaProdutosStyle';
import { TbHeart, TbShoppingCart, TbShoppingCartFilled } from 'react-icons/tb';
...
function CarouselLancamentos() {
  ...
  const { counterSlideLancamentos, nextSlideLancamentos, prevSlideLancamentos } = useCarouselLancamentos(produtosLancamentos.length);
  ...
  const handleLinkDetail = (nomeProduto) => {
    if (nomeProduto === 'Não contém produto') {
  ...
  return (
    <ContainerCarousel>
      <TituloCarousel>Lançamentos</TituloCarousel>
      <Carousel>
        {/* Seta estilizada para voltar à imagem anterior */}
        <PrevCarousel onClick={prevSlideLancamentos}>&#10094;</PrevCarousel>
        <CarouselHidden>
          <CarouselSlide style={{ transform: `translateX(${-counterSlideLancamentos * slideWidthGap}rem)` }}>
            {/* Mapeamento dos produtos da categoria "Lançamentos" */}
            {produtosLancamentos.map((produto, index) => (
              <ContainerImagemDados key={produto.id_produto || index} onClick={() => handleLinkDetail(produto.nome_produto)}>
                ...
                <TxtCarousel>{formatCurrencyBr(produto.preco_produto)}</TxtCarousel>
                <TxtCarousel>{formatParcelas(produto.parcela_produto)}</TxtCarousel>
        ...
        <NextCarousel onClick={nextSlideLancamentos}>&#10095;</NextCarousel>
      </Carousel>
    </ContainerCarousel>
  );
}

export default CarouselLancamentos;
