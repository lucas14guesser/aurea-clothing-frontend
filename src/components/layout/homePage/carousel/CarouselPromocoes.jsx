...
import { useCarouselPromocoes } from '../../../functions/homePage/carousel/CarouselFunctions'
...
function CarouselPromocoes() {
  ...
  const { counterSlidePromocoes, nextSlidePromocoes, prevSlidePromocoes } = useCarouselPromocoes(produtosPromocoes.length)
    ...
    <ContainerCarousel>
      ...
      <Carousel>
        ...
        <CarouselHidden>
          ...
          <CarouselSlide style={{ transform: `translateX(${-counterSlidePromocoes * slideWidthGap}rem)` }}>
            {/* Mapeamento dos produtos da categoria "promocoes" */}
            {produtosPromocoes.map((produto, index) => (
              <ContainerImagemDados key={produto.id_produto || index} onClick={() => handleLinkDetail(produto.nome_produto)}>
                ...
                <TxtGerais style={{ textDecoration: 'line-through' }}>{formatCurrencyBr(produto.preco_produto)}</TxtGerais>
                <TxtGerais style={{ color: '#A87826' }}>{formatCurrencyBr(produto.preco_promocional_produto)}</TxtGerais>
        ...
        {/* Seta estilizada com função de click. - Ao ser clicada ela avança para a próxima imagem do carrossel. */}
        <NextCarousel onClick={nextSlidePromocoes}>&#10095;</NextCarousel>
      </Carousel>
    </ContainerCarousel>
  )
}
