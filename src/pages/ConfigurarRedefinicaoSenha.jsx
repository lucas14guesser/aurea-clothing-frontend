...
function ConfigurarRedefinicaoSenha() {
    ...
    const handleSolicitarRedefinicao = async () => {
        ...
        try {
            const resp = await axios.post(`https://test-aureaclothing-backend-466bc65ebfec.herokuapp.com/aurea/usuario/${user.id_user}/solicitar-redefinicao-senha`);
    ...
    if (user.funcao_user === 'admin') {
        return (
            <React.Fragment>
                ...
                <ContainerDashBoard>
                    <FormRedefinir style={{ width: '50%' }}>
                        <Titulo>Redefinir Senha</Titulo>
                        <TxtGerais>Para redefinir a sua senha, clique no botão abaixo e confirme sua solicitação em seu e-mail.</TxtGerais>
                        ...
                        <ButtonRedefinir
                            ...
                            disabled={loading}
                        >
                            {loading ? 'Solicitando...' : 'Solicitar redefinição de senha'}
                        </ButtonRedefinir>
...
export default ConfigurarRedefinicaoSenha;
