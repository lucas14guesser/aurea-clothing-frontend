...
import { useCarouselJeans } from '../../../functions/homePage/carousel/CarouselFunctions'
...
function CarouselJeans() {
  const { user } = useContext(UserContext);

  const [produtos, setProdutos] = useState([]);
  const [produtosJeans, setProdutosJeans] = useState([]);
  ...
  const { counterSlideJeans, prevSlideJeans, nextSlideJeans } = useCarouselJeans(produtosJeans.length)
    ...
    <ContainerCarousel>
      ...
      <Carousel>
        ...
        <CarouselHidden>
          ...
          <CarouselSlide style={{ transform: `translateX(${-counterSlideJeans * slideWidthGap}rem)` }}>
            {/* Mapeamento dos produtos da categoria "jeans" */}
            {produtosJeans.map((produto, index) => (
        ...
        <NextCarousel onClick={nextSlideJeans}>&#10095;</NextCarousel>
      </Carousel>
    </ContainerCarousel>
  )
}

export default CarouselJeans
