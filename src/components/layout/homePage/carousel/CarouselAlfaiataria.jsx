...
import { useCarouselAlfaiataria } from '../../../functions/homePage/carousel/CarouselFunctions'
import { Subtitulo, TxtGerais } from '../../../../styles/GlobalStyles'
...
function CarouselAlfaiataria() {
    const { user } = useContext(UserContext);

    const [produtos, setProdutos] = useState([]);
    const [produtosAlfaiataria, setProdutosAlfaiataria] = useState([]);
    ...
    const { counterSlideAlfaiataria, prevSlideAlfaiataria, nextSlideAlfaiataria } = useCarouselAlfaiataria(produtosAlfaiataria.length);

    // UseEffect responsável por passar as imagens do carrossel automaticamente
    useEffect(() => {
        const intervaloId = setInterval(() => {
            nextSlideAlfaiataria();
        }, 6000);
    ...
    return (
        <ContainerCarousel>
            <TituloCarousel>Alfaiataria</TituloCarousel>
            <Carousel>
                {/* Seta estilizada com função de click. - Ao ser clicada ela volta para a imagem anterior do carrossel. */}
                <PrevCarousel onClick={prevSlideAlfaiataria}>&#10094;</PrevCarousel>
                <CarouselHidden>
                    <CarouselSlide style={{ transform: `translateX(${-counterSlideAlfaiataria * slideWidthGap}rem)` }}>
                        {/* Mapeamento dos produtos da categoria "alfaiataria" */}
                        {produtosAlfaiataria.map((produto, index) => (
                ...
                {/* Seta estilizada com função de click. - Ao ser clicada ela avança para a próxima imagem */}
                <NextCarousel onClick={nextSlideAlfaiataria}>&#10095;</NextCarousel>
            </Carousel>
        </ContainerCarousel>
    )
}

export default CarouselAlfaiataria
